...
class FlightVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        if (self.arrayFlightDetails.count != 0) {
            let flightName = self.arrayFlightDetails[section] as! NSDictionary
            return flightName["airline_code"] as? String
        }
        return ""
    }
...
extension FlightVC {
    func callWebServiceForFlightDetails() {
       // ShowMBProgressHUD(self.navigationController!, lableTitle: nil)
        WebServiceCalls.callWebservice(apiPath: "", method: .get, header: [:], params: [:], options: ["showProgres" : false]) { (error, response) in
            //hideMBProgressHUD(self.navigationController!)
